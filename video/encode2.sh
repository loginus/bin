...
then
	...
	DUMMY_BITRATE=1024
	mencoder -nosound $INPUT -o /dev/null  -ovc x264 -x264encopts bitrate=$DUMMY_BITRATE:pass=1:subq=1:frameref=1:threads=2; 
fi
...
mencoder -nosound $INPUT -o "$OUTPUT"  -ovc x264 -x264encopts bitrate=$BITRATE:pass=2:subq=6:frameref=15:partitions=all:me=umh:threads=2;
